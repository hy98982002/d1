...
- **/CHANGE_SUMMARY.md**
  用于记录「**当前这一次修改的执行摘要**」，面向：
  - 新接手的开发者
  - AI Agent 在后续任务中的上下文理解

  CHANGE_SUMMARY.md 的职责包括：
  - 本次修改的目标与背景（简述）
  - 本次修改涉及的文件列表（文件名级）
  - 新增 / 删除的文件（文件名 + 一句话说明）
  - 当前修改状态（是否稳定 / 是否存在已知问题）

  CHANGE_SUMMARY.md 仅描述「**本次修改的执行与现状**」，
  不作为历史记录，不替代 CHANGELOG.md，
  且在下一次修改完成后可被覆盖或重写。
...
<skill>
<name>doc-governance</name>
<description>Enforce strict separation between project memory (CHANGELOG) and project constitution (CLAUDE). Use this skill when documenting or updating project structure, rules, or architecture.</description>
<location>global</location>
</skill>

<skill>
<name>doc-governance-cn</name>
<description>强制区分项目“记忆（CHANGELOG）”与“宪法（CLAUDE）”，用于长期 SaaS / 平台型项目的文档规范治理。</description>
<location>global</location>
</skill>
...
<skill>
<name>frontend-design-cn</name>
<description>创建具有强烈个性、可投入生产的前端界面，具备高设计质量。当前端需求出现时使用此 Skill，例如构建网页组件、页面或应用。生成具有创意且精致的代码，避免“AI 稀释风格”的通用审美。</description>
<location>global</location>
</skill>

<skill>
<name>google-seo</name>
<description>Google Search Central documentation for SEO. Use when building websites and need guidance on search engine optimization, crawling, indexing, structured data, sitemaps, and search performance.</description>
<location>global</location>
</skill>
...
<skill>
<name>worktree-cn</name>
<description>规范 UAI_project 多 worktree 并行开发、合并与发布流程，确保职责隔离、历史清晰、可回滚。</description>
<location>global</location>
</skill>

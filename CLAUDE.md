...
本项目在与 AI Agent 协作时，使用独立的执行协议文档，
用于约定 AI 在任务完成后的收尾行为（如日志生成等）。

- AI 执行协议位于：`/docs/ai/00-00每次必用的结尾提示词.md`
- 该类文档不属于宪法规则本身
- 不具备项目约束力，仅用于规范 AI 行为
- 其内容不得反向修改或覆盖 `/CLAUDE.md` 中的任何规则

当 AI Agent 被用于修改代码或文档时，
应优先遵循对应的 AI 执行协议。

在使用 AI Agent 进行任务时，
项目通常配合使用位于 `/docs/ai/00-00每次必用的结尾提示词.md` 下的执行协议文档，
以提高修改记录的一致性与可追溯性。

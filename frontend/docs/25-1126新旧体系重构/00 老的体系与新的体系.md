...
- 商业策略更新：
  ...
  4. 「会员专区」含全部 Intermediate 课程，并新增 10 个会员专享课（属于 Intermediate，可单买但价格更高）。
...
| **Pathway（路径关系）** | ✅ 必须  | ⭐⭐⭐⭐⭐     | 高   | 学习路线图（/programs/ 学习体系实体）必须使用结构性关系建模： - `/programs/` → 子节点：使用 `hasPart` - 子课程 → `/programs/`：使用 `isPartOf` |

说明： - `/programs/` 对应 Schema 类型为 `EducationalOccupationalProgram` - 学习路线是“结构关系”，而不是“课程列表” - `hasCourse` 仅作为可选补充字段，不作为主干 AEO 关系

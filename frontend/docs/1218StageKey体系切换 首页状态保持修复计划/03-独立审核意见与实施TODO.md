...
结论：
- 关键决策点都要实现
- phase 1 , phase 2 可以立即执行
- 本项目目前是 pnpm dev.
- 文档化建议里，继续创建04*,05*.

- 推荐在 Phase 1 就将 Feature Flag 写好并随代码一同上线。即便默认值为 true（启用新体系），也能在出现紧急 Bug 时立刻切到 false，避免回滚构建和额外部署。

后续一旦监控稳定、残留 basic 字符串全部清理完毕，再考虑彻底移除回滚逻辑即可。

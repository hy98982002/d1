...
**前端实现要点**：
- 阶段体系已彻底收敛为 Beginner/intermediate/advanced
...
### 前端补充约束
- 前端相关文件（`.vue` `.ts` `.md` `.skill.md` `.spec.md` 等）
  **严禁** 使用 Windows 非法字符：
  `* ? : < > | " / \`
- 路由、slug、文档文件名 **必须** 使用安全字符集：
  `a-z 0-9 - _`
- PRD / 技术文档文件名 **不得** 使用“最终 / last / final”等语义

> 前端文件命名不合规，将直接导致 Windows 协作失败，
> 属于 **阻断级错误**。
...
- 体系已彻底收敛为 Beginner/intermediate/advanced，`StageMeta`/`StageKeySchema` 提供唯一数据源与运行时校验，禁止新增旧阶段或引入旧 → 新映射逻辑。【F:src/types/index.ts】【F:src/utils/stageMap.ts】
...
│       ├── SettingBeginnerInfo.vue         # Beginner information settings
...
└── images/                   # 业务图片
    └── courses/              # 课程图片
        ├── photoshop-Beginner-cover-480.png     # 基础课程
        ├── python-intermediate-cover-1280.webp   # 进阶课程
        ├── ai-designer-advanced-cover-1920.png   # 高阶课程
        └── photoshop-membership-Beginner-cover-480.png # 会员专属课程
```
...
- **难度阶段**: 必须为 `Beginner`、`intermediate` 或 `advanced` 之一
- **尺寸**: 如 `480`, `1280`, `1920` 等，对应不同分辨率
- **格式**: `png` 或 `webp`

**示例**:

- `photoshop-Beginner-cover-480.png` (Photoshop 基础课程，480px 分辨率，PNG 格式)
...
- 课程阶段已统一收敛为 `Beginner`/`intermediate`/`advanced` 三级体系
